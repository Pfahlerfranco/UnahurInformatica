---
title: Licenciatura en Informática
permalink: /licenciatura-informatica/
icono: /assets/img/Informatica-licenciatura-icono.png
descripcion_corta: Licenciatura con título intermedio Técnico/a Universitario en Programación.
imagen: assets/images/licenciatura.jpg
vigente: true
titulo_intermedio: Técnico/a Universitario en Programación
total_horas: 3521
pdf: /assets/pdf/planes/licenciatura-informatica-unahur.pdf
tabla_datos:
  col_grado: Licenciado/a en Informática
  col_intermedio: Técnico/a Universitario en Programación
  filas:
    - label: Duración
      grado: 4,5 años
      intermedio: 2,5 años
    - label: Horas Interacción pedagógica
      grado: "3200"
      intermedio: "1184"
    - label: Materias
      grado: "45"
      intermedio: "17"
    - label: Créditos ACA
      grado: "30"
      intermedio: "12"
    - label: Créditos Totales
      grado: "287"
      intermedio: "120"
layout: carrera
orden: 1
cuatrimestres:
  - nombre: Primer Año
    horas: ""
    materias:
      - nombre: Matemática
        slug: matematicaI
        area: CB
        hs_sem: 8
        hs_total: 128
        correlativas: "-"
      - nombre: Introducción a la Programación
        slug: introduccionProgramacion
        area: AyL
        hs_sem: 8
        hs_total: 128
        correlativas: "-"
      - nombre: Organización de Computadoras
        slug: organizacionComputadoras
        area: ASOyR
        hs_sem: 6
        hs_total: 96
        correlativas: "-"
      - nombre: Nuevos Entornos y Lenguajes
        slug: nuevosEntornos
        area: Otros
        hs_sem: 2
        hs_total: 32
        correlativas: "-"
      - nombre: Estructuras de Datos
        slug: estructuraDatos
        area: TC
        hs_sem: 8
        hs_total: 128
        correlativas: "-"
      - nombre: Programación con Objetos I
        slug: programacionObjetosI
        area: AyL
        hs_sem: 8
        hs_total: 128
        correlativas: "-"
      - nombre: Bases de Datos
        slug: baseDatos
        area: ISBDySI
        hs_sem: 6
        hs_total: 96
        correlativas: "-"
      - nombre: Inglés I
        slug: inglesI
        area: Otros
        hs_sem: 2
        hs_total: 32
        correlativas: "-"
  - nombre: Segundo Año
    horas: ""
    materias:
      - nombre: Matemática II
        slug: matematicaII
        area: CB
        hs_sem: 4
        hs_total: 64
        correlativas: "-"
      - nombre: Programación con Objetos II
        slug: programacionObjetosII
        area: AyL
        hs_sem: 6
        hs_total: 96
        correlativas: Programación con Objetos I
      - nombre: Redes de Computadoras
        slug: redesComputadoras
        area: ASOyR
        hs_sem: 6
        hs_total: 96
        correlativas: "-"
      - nombre: Sistemas Operativos
        slug: sistemasOperativos
        area: ASOyR
        hs_sem: 6
        hs_total: 96
        correlativas: "-"
      - nombre: Programación Funcional
        slug: programacionFuncional
        area: AyL
        hs_sem: 4
        hs_total: 64
        correlativas: "-"
      - nombre: Construcción de Interfaces de Usuario
        slug: construccionInterfacesUsuario
        area: ISBDySI
        hs_sem: 6
        hs_total: 96
        correlativas: "-"
      - nombre: Algoritmos
        slug: algoritmos
        area: AyL
        hs_sem: 6
        hs_total: 96
        correlativas: "-"
      - nombre: Estrategias de Persistencia
        slug: estrategiasPersistencia
        area: ISBDySI
        hs_sem: 6
        hs_total: 96
        correlativas: "-"
      - nombre: Laboratorio de Sistemas Operativos y Redes
        slug: laboratorioSistemasOperativosRedes
        area: ASOyR
        hs_sem: 4
        hs_total: 64
        correlativas: "-"
  - nombre: Tercer Año
    horas: ""
    materias:
      - nombre: Análisis Matemático
        slug: analisisMatematico
        area: CB
        hs_sem: 6
        hs_total: 96
        correlativas: "-"
      - nombre: Lógica y Programación
        slug: logicaProgramacion
        area: TC
        hs_sem: 6
        hs_total: 96
        correlativas: "-"
      - nombre: Elementos de Ingeniería de Software
        slug: elementosIngenieriaSoftware
        area: ISBDySI
        hs_sem: 6
        hs_total: 96
        correlativas: "-"
      - nombre: Seguridad de la Información
        slug: seguridadInformacion
        area: ASOyR
        hs_sem: 4
        hs_total: 64
        correlativas: "-"
      - nombre: Materia UNAHUR I
        slug: asignaturaUnahurI
        area: Otros
        hs_sem: 2
        hs_total: 32
        correlativas: "-"
      - nombre: Inglés II
        slug: inglesII
        area: Otros
        hs_sem: 2
        hs_total: 32
        correlativas: Inglés I
      - nombre: Matemática III
        slug: matematicaIII
        area: CB
        hs_sem: 4
        hs_total: 64
        correlativas: "-"
      - nombre: Programación Concurrente
        slug: programacionConcurrente
        area: AyL
        hs_sem: 4
        hs_total: 64
        correlativas: "-"
      - nombre: Ingeniería de Requerimientos
        slug: ingenieriaRequerimientos
        area: ISBDySI
        hs_sem: 4
        hs_total: 64
        correlativas: "-"
      - nombre: Desarrollo de Aplicaciones
        slug: desarrolloAplicaciones
        area: ISBDySI
        hs_sem: 6
        hs_total: 96
        correlativas: "-"
  - nombre: Cuarto Año
    horas: ""
    materias:
      - nombre: Probabilidad y Estadística
        slug: probabilidadEstadistica
        area: CB
        hs_sem: 6
        hs_total: 96
        correlativas: "-"
      - nombre: Gestión de Proyectos de Desarrollo de Software
        slug: gestionProyectosDesarrolloSoftware
        area: ISBDySI
        hs_sem: 4
        hs_total: 64
        correlativas: "-"
      - nombre: Lenguajes Formales y Autómatas
        slug: lenguajesFormalesAutomatas
        area: TC
        hs_sem: 4
        hs_total: 64
        correlativas: "-"
      - nombre: Programación con Objetos III
        slug: programacionObjetosIII
        area: AyL
        hs_sem: 4
        hs_total: 64
        correlativas: Programación con Objetos II
      - nombre: Materia UNAHUR II
        slug: asignaturaUnahurII
        area: Otros
        hs_sem: 2
        hs_total: 32
        correlativas: "-"
      - nombre: Práctica Profesional Supervisada (PPS)
        slug: practicaProfesionalSupervisada
        area: Otros
        hs_sem: 6
        hs_total: 96
        correlativas: "-"
      - nombre: Teoría de la Computación
        slug: teoriaComputacion
        area: TC
        hs_sem: 4
        hs_total: 64
        correlativas: "-"
      - nombre: Arquitectura de Software I
        slug: arquitecturaSoftwareI
        area: ISBDySI
        hs_sem: 4
        hs_total: 64
        correlativas: "-"
      - nombre: Sistemas Distribuidos y Tiempo Real
        slug: SistemasDistribuidosTiempoReal
        area: ASOyR
        hs_sem: 6
        hs_total: 96
        correlativas: "-"
  - nombre: Quinto Año
    horas: ""
    materias:
      - nombre: Tesina de Licenciatura
        slug: tesinaLicenciatura
        area: Otros
        hs_sem: 5
        hs_total: 160
        correlativas: "-"
      - nombre: Materia Optativa I
        slug: optativaI
        area: Otros
        hs_sem: 4
        hs_total: 64
        correlativas: "-"
      - nombre: Características de Lenguajes de Programación
        slug: caracteristicasLenguajes
        area: AyL
        hs_sem: 4
        hs_total: 64
        correlativas: "-"
      - nombre: Arquitectura de Software II
        slug: arquitecturaSoftwareII
        area: ISBDySI
        hs_sem: 4
        hs_total: 64
        correlativas: Arquitectura de Software I
      - nombre: Arquitectura de Computadoras
        slug: arquitecturaComputadoras
        area: ASOyR
        hs_sem: 4
        hs_total: 64
        correlativas: "-"
      - nombre: Materia Optativa II
        slug: MateriaComplementariaSistemasInformacionGeografica
        area: Otros
        hs_sem: 4
        hs_total: 64
        correlativas: "-"
      - nombre: Parseo y generación de código
        slug: parseoGeneracionCodigo
        area: AyL
        hs_sem: 4
        hs_total: 64
        correlativas: "-"
      - nombre: Ejercicio Profesional
        slug: ejercicioProfesional
        area: APyS
        hs_sem: 3
        hs_total: 48
        correlativas: "-"
      - nombre: Tecnología y Sociedad
        slug: tecnologiaSociedad
        area: APyS
        hs_sem: 3
        hs_total: 48
        correlativas: "-"
tecnicatura_inteligencia_artificial:
  titulo: Tecnicatura Universitaria en Inteligencia Artificial
  descripcion: Formación en fundamentos de inteligencia artificial, ciencia de
    datos y aprendizaje automático.
  imagen: assets/images/planes-ia.jpg
  link: /tecnicatura-inteligencia-artificial/
  externo: false
  total_horas: 1472
  creditos: 35
  cuatrimestres:
    - nombre: Primer Cuatrimestre
      horas: 224
      materias:
        - nombre: Matemática para informática I
          slug: matematicaInformaticaI
          area: CB
          hs_sem: 4
          hs_total: 64
          correlativas: "-"
        - nombre: Introducción a lógica y problemas computacionales
          slug: introduccionLogicaProblemasComputacionales
          area: AyL
          hs_sem: 4
          hs_total: 64
          correlativas: "-"
        - nombre: Introducción a la inteligencia artificial
          slug: introduccionInteligenciaArtificial2.1
          area: AyL
          hs_sem: 4
          hs_total: 64
          correlativas: "-"
        - nombre: "Nuevos entornos y lenguajes: la producción del conocimiento en la
            cultura digital"
          slug: nuevosEntornos
          area: Gral.
          hs_sem: 2
          hs_total: 32
          correlativas: "-"
    - nombre: Segundo Cuatrimestre
      horas: 272
      materias:
        - nombre: Álgebra lineal
          slug: algebraLineal
          area: CB
          hs_sem: 4
          hs_total: 64
          correlativas: Matemática para informática I
        - nombre: Cálculo
          slug: calculo
          area: CB
          hs_sem: 4
          hs_total: 64
          correlativas: Matemática para informática I
        - nombre: Taller de Programación I
          slug: tallerProgramacionI
          area: AyL
          hs_sem: 4
          hs_total: 64
          correlativas: Introducción a lógica y problemas computacionales
        - nombre: Tecnología y sociedad
          slug: tecnologiaSociedad
          area: APyS
          hs_sem: 3
          hs_total: 48
          correlativas: "-"
        - nombre: Inglés I
          slug: inglesI
          area: Gral.
          hs_sem: 2
          hs_total: 32
          correlativas: "-"
    - nombre: Tercer Cuatrimestre
      horas: 320
      materias:
        - nombre: Bases de datos
          slug: baseDatos
          area: ISBDySI
          hs_sem: 6
          hs_total: 96
          correlativas: Álgebra lineal
        - nombre: Probabilidad y estadística
          slug: probabilidadEstadistica
          area: CB
          hs_sem: 6
          hs_total: 96
          correlativas: Álgebra lineal,Cálculo
        - nombre: Taller de programación II
          slug: tallerProgramacionII
          area: AyL
          hs_sem: 4
          hs_total: 64
          correlativas: Taller de Programación I
        - nombre: Fundamentos de redes neuronales
          slug: fundamentosRedesNeuronales
          area: IA
          hs_sem: 4
          hs_total: 64
          correlativas: Introducción a la inteligencia artificial,Álgebra lineal,Taller de
            Programación I
    - nombre: Cuarto Cuatrimestre
      horas: 272
      materias:
        - nombre: Fundamentos de ciencias de datos
          slug: fundamentosCienciaDatos
          area: IA
          hs_sem: 3
          hs_total: 48
          correlativas: Introducción a la inteligencia artificial,Álgebra lineal,Cálculo
        - nombre: Aprendizaje automático
          slug: aprendizajeAutomatico
          area: IA
          hs_sem: 4
          hs_total: 64
          correlativas: Probabilidad y estadística,Fundamentos de redes neuronales
        - nombre: Electiva
          slug: electivaInteligenciaArtificial
          area: Elec.
          hs_sem: 4
          hs_total: 64
          correlativas: Probabilidad y estadística,Fundamentos de redes neuronales
        - nombre: Taller de programación III
          slug: tallerProgramacionIII
          area: AyL
          hs_sem: 4
          hs_total: 64
          correlativas: Taller de programación II
        - nombre: Inglés II
          slug: inglesII
          area: Gral.
          hs_sem: 2
          hs_total: 32
          correlativas: Inglés I
    - nombre: Quinto Cuatrimestre
      horas: 304
      materias:
        - nombre: Materia UNAHUR
          slug: asignaturaUnahurI
          area: Gral.
          hs_sem: 2
          hs_total: 32
          correlativas: "-"
        - nombre: Aprendizaje automático avanzado
          slug: aprendizajeAutomaticoAvanzado
          area: IA
          hs_sem: 6
          hs_total: 96
          correlativas: Aprendizaje automático,Taller de programación III
        - nombre: Procesamiento de Imágenes y Visión por Computadora
          slug: procesamientoImagenesVisionComputadora
          area: IA
          hs_sem: 6
          hs_total: 96
          correlativas: Aprendizaje automático,Taller de programación III
        - nombre: Proyecto integrador
          slug: proyectoIntegrador
          area: IA
          hs_sem: 5
          hs_total: 80
          correlativas: Aprendizaje automático,Taller de programación III
creditos: 0
---
